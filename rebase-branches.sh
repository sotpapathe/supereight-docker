...
# Ubuntu 22.04
git branch --force ubuntu2204 master

# Ubuntu 20.04
git branch --force ubuntu2004 master
git checkout ubuntu2004
sed --in-place 's/ARG BASE_IMAGE=ubuntu:22\.04/ARG BASE_IMAGE=ubuntu:20\.04/' Dockerfile
git add Dockerfile
git commit --message 'Change default image to Ubuntu 20.04'

# ROS Noetic
git branch --force ros-noetic master
git checkout ros-noetic
sed --in-place 's/ARG BASE_IMAGE=ubuntu:22\.04/ARG BASE_IMAGE=ros:noetic-ros-base-focal/' Dockerfile
git add Dockerfile
...
sed --in-place 's/ARG BASE_IMAGE=ubuntu:22\.04/ARG BASE_IMAGE=ubuntu:18\.04/' Dockerfile
git add Dockerfile
...
sed --in-place 's/ARG BASE_IMAGE=ubuntu:22\.04/ARG BASE_IMAGE=ros:melodic-ros-base-bionic/' Dockerfile
git add Dockerfile
...
if [ "$#" -eq 1 ] && [ "$1" = 'push' ]; then
	...
	git push --force origin ubuntu2204:ubuntu2204
